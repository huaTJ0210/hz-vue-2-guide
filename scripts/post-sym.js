...
// sentry-ci可执行程序文件路径
const sentryCliPath = `${path.join(__dirname, '..', 'node_modules', '.bin', 'sentry-cli')}`

// 检测当前应用是否开启了source-map
// if (devtool.indexOf('source-map') === -1) {
//   throw Error(`source-map必须开启! 请检查当前webpack.config.prod下的devtool值是否为source-map。当前devtool值为: ${devtool}`)
// }
...
function uploadSourcemapsFile(sourcemapsDir) {
  ...
  function uploadCore() {
    ...
    exec(createRelease, (err, output) => {
      if (err) {
        throw Error(`创建名为: "${release}"版本号时，出现错误，错误输出为:\n${output}`)
      }
